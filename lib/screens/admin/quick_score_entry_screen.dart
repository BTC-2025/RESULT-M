...
class QuickScoreEntryScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<QuickScoreEntryScreen> createState() => _QuickScoreEntryScreenState();
}
...
class _QuickScoreEntryScreenState extends ConsumerState<QuickScoreEntryScreen> {
  ...
  Future<void> _fetchRecord() async {
    ...
    try {
      final record = await ref.read(apiServiceProvider).getDatasetRecord(widget.recordId);
  ...
  Future<void> _saveScore({bool isAutoSave = false}) async {
    ...
    try {
      ...
      final updatedRecord = await ref.read(apiServiceProvider).updateDatasetRecord(
        widget.datasetId,
        widget.recordId,
        updatedData,
        _currentVersion,
      ...
      _initializeData(updatedRecord); // Resets _hasUnsavedChanges and updates version

      if (!isAutoSave && mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Score updated!'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
  ...
  String _formatKey(String key) {
    ...
    return key.replaceAll(RegExp(r'(?<=[a-z])(?=[A-Z])'), ' ')
              .replaceAll('_', ' ')
              .toUpperCase();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('QUICK UPDATE', style: TextStyle(fontWeight: FontWeight.w900, letterSpacing: 1.2, fontSize: 16)),
        ...
        actions: [
          if (_hasUnsavedChanges)
            const Padding(
              ...
              child: Center(
                child: Text('Unsaved', style: TextStyle(color: Colors.orangeAccent, fontWeight: FontWeight.bold, fontSize: 12)),
              ),
            ),
        ],
      ),
      ...
      children: [
        ...
        Container(
          ...
          child: Column(
            ...
            children: [
              Text(_originalRecord?['recordTitle'] ?? 'Record Data', style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 18, color: Color(0xFF0F172A))),
              const SizedBox(height: 12),
              ..._textFields.entries.map((e) => Padding(
                padding: const EdgeInsets.only(bottom: 6),
                child: Row(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('${_formatKey(e.key)}: ', style: const TextStyle(color: Colors.grey, fontWeight: FontWeight.bold, fontSize: 13)),
                    Expanded(child: Text('${e.value}', style: const TextStyle(color: Color(0xFF0F172A), fontWeight: FontWeight.bold, fontSize: 13))),
                  ],
                ),
              )).toList(),
            ],
        ...
        const Text('EDIT SCORES', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.w900, letterSpacing: 1.5)),
        const SizedBox(height: 16),
        if (_numericFields.isEmpty)
          const Text('No numeric fields available to edit.', style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic)),
        ..._numericFields.entries.map((e) => _buildNumericRow(e.key, e.value)).toList(),
      ],
    );
  }
  ...
  Widget _buildNumericRow(String key, num value) {
    return Container(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text(_formatKey(key), style: const TextStyle(fontWeight: FontWeight.w900, color: Colors.grey, fontSize: 12, letterSpacing: 1.2)),
          const SizedBox(height: 12),
          Row(
            children: [
              _buildControlButton(Icons.remove, () => _incrementValue(key, -1), onLongPress: () => _incrementValue(key, -10)),
              Expanded(
                child: Padding(
                  ...
                  child: TextField(
                    ...
                    style: const TextStyle(fontWeight: FontWeight.w900, fontSize: 28, color: Color(0xFF0F172A)),
                    decoration: const InputDecoration(border: InputBorder.none, isDense: true, contentPadding: EdgeInsets.zero),
                    onChanged: (val) => _setValue(key, val),
                  ),
                ),
              ),
              _buildControlButton(Icons.add, () => _incrementValue(key, 1), onLongPress: () => _incrementValue(key, 10)),
            ],
          ),
        ],
      ),
  ...
  Widget _buildControlButton(IconData icon, VoidCallback onTap, {VoidCallback? onLongPress}) {
    return Material(
      ...
      child: SafeArea(
        child: ElevatedButton(
          ...
          child: _isSaving
              ...
              : const Text('Update Score', style: TextStyle(color: Colors.white, fontSize: 16, fontWeight: FontWeight.w900, letterSpacing: 1)),
        ),
      ),
    );
  }
